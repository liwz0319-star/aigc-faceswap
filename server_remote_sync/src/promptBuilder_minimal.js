...
const scenes = require('./data/scenes_minimal');
 ...
 */
/**
 * 最简单人提示词（场景1/2/3：仅球迷，场景3：球迷+Bernie）
 */
function buildMinimalSoloPrompt(sceneId, userDescription, options) {
  const scene = scenes[sceneId];
  ...
  const isBernie = sceneId === '3';

  const openingLine = isBernie
    ? `Photorealistic photo of exactly 2 subjects (1 fan + 1 bear mascot). Tall portrait orientation. Real photograph. 图片长宽比4:5`
    : `Photorealistic solo portrait photo of exactly 1 person. Tall portrait orientation. Real photograph. Full body from head to feet visible. 图片长宽比4:5`;

  const clothing = scene.clothing_minimal || scene.clothing_adult || '';
  const action = scene.action_minimal || '';
  const environment = scene.environment || '';

  const jerseyRefLine = jerseyIdx > 0
    ? ` JERSEY REFERENCE: Reference image ${jerseyIdx} shows the exact jersey — copy this design precisely.`
    : '';

  const settingLine = bgIdx > 0
    ? `SETTING (match reference image ${bgIdx} — reproduce the same location, lighting, and atmosphere):\n${environment}`
    : `SETTING:\n${environment}`;

  const userImageRef = userImageCount === 1
    ? `Reference image 1: THE FAN — reproduce this face exactly. CRITICAL EYE RULE: reproduce EXACT same eye size, shape, openness as in reference image 1 — do NOT shrink or narrow the eyes. Eyes fully open and natural. Only include glasses if the reference shows them.`
    : `Reference images 1 through ${userImageCount}: ALL show THE SAME PERSON (the fan). Reproduce this person's face exactly. CRITICAL EYE RULE: reproduce EXACT same eye size, shape, openness — do NOT shrink eyes. Eyes fully open and natural. Use ALL ${userImageCount} images as face reference.`;

  const faceRule = `FACE (HIGHEST PRIORITY): Fan's face MUST exactly match reference image 1 — identical features, skin tone, face shape, hair. EYE RULE: The fan's eye size, shape, and opening MUST match reference image 1 exactly — do NOT make eyes smaller or narrower. MOUTH RULE: CLOSED-MOUTH gentle smile — NOT open mouth, NOT showing teeth. Do NOT copy clothing from reference images.`;

  const subjectSection = isBernie
    ? `THE SUBJECTS (exactly 2):\n1. THE FAN — ${userDescription}. Face matches reference image 1. CLOSED-MOUTH smile.\n2. BERNIE THE BEAR MASCOT — FC Bayern Munich's official large brown bear mascot in full bear costume — round bear head, brown furry body.`
    : `THE SUBJECT:\nTHE FAN — ${userDescription}. Face matches reference image 1. CLOSED-MOUTH smile. Full-grown ADULT.`;

  const qualityTag = `Photorealistic, sharp focus on face, 8K quality.`;

  const prompt = `${openingLine}

CLOTHING:
${clothing}${jerseyRefLine}
...
${qualityTag}`;
  ...
  return {
    prompt,
    player_names: [],
    native_params: scene.native_params_minimal || scene.native_params || {},
  };
}

function buildAllPrompts(starIds, sceneId, userMode, userDescription, options = {}) {
  // minimal 单人模式：场景1/2/3仅球迷，不需要球员参数
  if (options.minimalSolo) {
    return buildMinimalSoloPrompt(sceneId, userDescription, options);
  }
...
⚠️ CLOTHING OVERRIDE — READ BEFORE ANYTHING ELSE: The fan's reference photos show civilian/casual clothes. YOU MUST COMPLETELY IGNORE THE FAN'S REFERENCE PHOTO CLOTHING. The fan MUST wear the EXACT SAME FC Bayern Munich kit as the players — bright red jersey, white stripes, white T logo. NO gray jacket, NO black top, NO casual clothes on the fan under any circumstances.

CLOTHING (apply first — all ${clothingCount} wear identical outfits):
${clothing}${jerseyRefLine} Every human person — INCLUDING THE FAN — wears this exact same outfit. OVERRIDE any clothing seen in reference photos.
