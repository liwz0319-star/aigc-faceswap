/**
 * Faceswap 场景2 自动化测试脚本
 * 流程:
 *   1. 视觉模型识别用户性别
 *   2. 根据性别选择模板:
 *      - 男性 → scene_02_1777013168257.png       (右边第二个人)
 *      - 女性 → scene_02_user2_1777014143898.png (右边第二个人)
 *   3. Faceswap 生成 + 提示词记录
 *
 * 用法:
 *   node test-faceswap-scene2.js [用户照片路径]
 *   node test-faceswap-scene2.js                          # 使用默认照片
 *   node test-faceswap-scene2.js "生成测试/照片/xxx.jpg"   # 指定照片
 *   node test-faceswap-scene2.js --gender male            # 强制指定性别
 *   node test-faceswap-scene2.js --gender female          # 强制指定性别
 */
...
const SCENE2_TEMPLATES = {
  male: {
    ...
    targetDesc:   '右边第二个人',
    description:  '男球迷模板: 球员 | 球员 | 男球迷(替换) | 球员',
  },
  female: {
    file: 'scene_02_user2_1777014143898.png',
    targetPerson: 'the second person from the right',
    targetDesc:   '右边第二个人',
    description:  '女球迷模板: 球员 | 球员 | 女球迷(替换) | 球员',
  },
};
...
function parseCliArgs(argv) {
  ...
  if (positionals[0]) options.userPhotoPath = positionals[0];
...
async function main() {
  ...
  console.log('========================================');
  console.log('Faceswap 场景2 自动化测试');
  console.log('========================================');
  ...
  // ── Step 1: 读取用户照片 ──
  console.log('\n[Step 1] 读取用户照片...');
  if (!fs.existsSync(userPhotoPath.replace(/\//g, '\\'))) {
    const altPath = userPhotoPath.replace(/\\/g, '/');
    if (!fs.existsSync(altPath)) {
      console.error(`用户照片不存在: ${userPhotoPath}`);
      process.exit(1);
    }
  }
  ...
  if (cli.gender) {
    console.log(`[性别检测] 使用命令行指定性别: ${cli.gender}`);
  } else {
    try {
  ...
  try {
    ...
    const localFile  = path.join(OUTPUT_DIR, `scene2_auto_${genderTag}_${Date.now()}.jpg`);
    await downloadFile(result.url, localFile);
    console.log(`已保存: ${path.basename(localFile)}`);

    // ── 保存提示词日志 ──
    const timestamp   = Date.now();
    const promptLogFile = path.join(PROMPT_LOG_DIR, `scene2_auto_${genderTag}_${timestamp}.json`);
    const promptLogData = {
    ...
    console.log(`提示词日志: ${path.basename(promptLogFile)}`);
