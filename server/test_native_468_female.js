 ...
 * 球星 4/6/8 (Kane, Lennart Karl, Neuer) × 场景 1/2/3/4 × 女性成人
 *
 * 用户照片: 生成测试/照片/1.jpg
 * 用法: node test_native_468_female.js
 */
...
// ─── 测试配置 ───
const STAR_IDS = ['4', '6', '8']; // Harry Kane, Lennart Karl, Manuel Neuer
const SCENE_IDS = ['2'];
const USER_MODE = 'adult';
const GENDER = 'female';
const USER_IMAGE_PATH = path.resolve(PROJECT_ROOT, '生成测试', '照片', '1.jpg');
...
async function runTest(sceneId, userImage, userDescription) {
  ...
  try {
    ...
    if (PROMPT_ONLY) {
      ...
      // 球星参考图（人脸锚定）
      const playerGroups = getPlayerReferenceImages(STAR_IDS);
      for (const player of playerGroups) {
        for (const ref of player.refs) {
      ...
      // 构图参考图（场景4专用：引导人物比例和互动姿势）
      const compImage = await loadCompositionReference(sceneId);
      if (compImage) {
        images.push(compImage);
        console.log(`  [构图参考图] 场景${sceneId} composition_reference`);
      }
    }

    console.log(`  [参考图] 总计: ${images.length} 张${PROMPT_ONLY ? ' (纯Prompt模式)' : ` (用户1 + 球星${STAR_IDS.length} + 其他 ${images.length - 1 - STAR_IDS.length})`}`);
    ...
    // 4. 下载图片
    const outputDir = path.resolve(PROJECT_ROOT, '生成测试', '结果', '球星468女');
...
async function main() {
  ...
  // 步骤1: 解读用户照片（LAS 视觉 API → 火山引擎视觉 → 参考图描述）
  console.log('\n' + '─'.repeat(60));
