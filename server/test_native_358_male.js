 ...
 * 球星 3/5/8 (Kimmich, Luis Díaz, Neuer) × 场景 1/2/3/4 × 男性成人
 *
 * 修复版：
 * - assetStore 路径修复（素材/参考图/）
 * - 加入合照参考图（composition_reference）
 * - 场景3 outfield 球衣强化（防止 Neuer 被生成守门员服）
 * - 场景4 球迷角色强化
 *
 * 用法: node test_native_358_male.js
 */
...
// ─── 测试配置 ───
const STAR_IDS = ['3', '5', '8']; // Joshua Kimmich, Luis Díaz, Manuel Neuer
const SCENE_IDS = ['1', '2', '3', '4'];
const USER_MODE = 'adult';
const GENDER = 'male';
const USER_IMAGE_PATH = path.resolve(PROJECT_ROOT, '生成测试', '照片', '用户照片-男.png');
  ...
  try {
    ...
    if (PROMPT_ONLY) {
      ...
      if (compositionImage) {
        ...
        console.log(`  [合照参考图] 已加载（场景${sceneId}）`);
      }
      ...
      if (jerseyImages.length > 0) {
        ...
        console.log(`  [球衣参考图] ${jerseyImages.length} 张`);
      }
      ...
      if (beerMugImage) {
        ...
        console.log(`  [酒杯参考图] 已加载`);
      }
    }

    console.log(`  [参考图] 总计: ${images.length} 张`);

    // 3. 调用 Seedream native API（使用场景专属 native_params，与生产代码对齐）
    const sceneSize = scene?.recommended_size || '1536x2560';
    const nativeParams = scene?.native_params || {};
    console.log(`  [参数] strength=${nativeParams.strength ?? IMAGE_STRENGTH} | guidance=${nativeParams.guidance_scale ?? GUIDANCE_SCALE}`);
    ...
    // 4. 下载图片
    const outputDir = path.resolve(PROJECT_ROOT, '生成测试', '结果', '球星358修复');
    ...
    const outputFile = path.resolve(outputDir, `球星${STAR_IDS.join('')}_场景${sceneId}_${modelTag}${modeTag}.png`);
...
async function main() {
  ...
  console.log(`  Native 模式批量测试（修复版）：球星[${STAR_IDS.join(',')}] × 全场景 × 男性成人`);
  console.log(`  修复：路径修复 + 合照参考图 + 场景3守门员服排除 + 场景4球迷强化`);
  console.log('='.repeat(60));
  ...
  console.log('\n' + '='.repeat(60));
  console.log('  测试结果汇总（修复版 / 球星358）');
  console.log('='.repeat(60));

  for (const r of results.sort((a, b) => parseInt(a.sceneId) - parseInt(b.sceneId))) {
    if (r.success) {
