/**
 * Seedream 模型调试测试脚本
 * 提示词来源: src/promptBuilder_seedream.js + src/data/scenes_seedream.json
 * 修改 scenes_seedream.json 或 promptBuilder_seedream.js 不影响生产提示词
 *
 * 用法: node test_seedream_debug.js
 */
...
const OUTPUT_DIR = path.resolve(__dirname, '..', '生成测试', 'seedream_debug');
...
function generatePrompts() {
  // ★ 使用 seedream 专属构建器，修改 promptBuilder_seedream.js / scenes_seedream.json 不影响生产
  const { buildAllPrompts } = require('./src/promptBuilder_seedream');
...
async function main() {
  ...
  console.log('\n' + D);
  console.log('  [SEEDREAM DEBUG] Karl(106) / Musiala(107) / Neuer(108)');
  console.log('  提示词来源: promptBuilder_seedream.js + scenes_seedream.json');
  console.log(D);
  ...
  console.log(D);
  console.log('  [SEEDREAM] 生成提示词（来自 seedream 专属文件）');
  console.log(D);
