 ...
 * 球星 7/8/9 (Musiala, Neuer, Pavlovic) × 场景 1/2/3/4 × 女性成人
 *
 * 直接调用 Seedream 4.5 官方 API，跳过服务器队列层
 * 用法: node test_native_789_female.js
 */
...
// ─── 配置 ───
const STAR_IDS = ['4', '8', '10'];
const SCENE_IDS = ['1', '2', '3', '4'];
const USER_MODE = 'adult';
const GENDER = 'male';
const USER_IMAGE_PATH = path.resolve(PROJECT_ROOT, '照片', '用户照片-男.png');
 ...
 */
async function callSeedreamNative({ prompt, images = [], size = '1664x1664', negative_prompt }) {
  const payload = {
    model: MODEL,
    prompt,
...
async function runTest(sceneId, userImage, userDescription) {
  ...
  console.log(`\n${'─'.repeat(60)}`);
  console.log(`[${timestamp()}] 开始: 球星[7,8,9] × ${testLabel}`);
  ...
  try {
    ...
    if (!scene?.skip_scene_ref) {
      ...
      console.log(`  [参考图] 场景${sceneId} 跳过（skip_scene_ref=true）`);
      // 即使跳过人物参考图，也加载背景参考图
    ...
    const outputFile = path.resolve(outputDir, `球星${STAR_IDS.join('')}_场景${sceneId}_${GENDER}_${modelTag}.png`);
...
async function main() {
  ...
  const playerNames = STAR_IDS.map(id => require('./src/data/players')[id]?.name || `#${id}`);
  console.log(`  Native 模式批量测试：球星[${STAR_IDS.join(',')}] × 场景[${SCENE_IDS.join(',')}] × ${GENDER === 'female' ? '女性' : '男性'}`);
  console.log('='.repeat(60));
