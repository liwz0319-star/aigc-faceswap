/**
 * 批量测试脚本
 * 测试球星 3/4/5 × 场景 1/2/3/4 共 12 种组合
 * 使用儿童照片，native 模式
 *
...
const CONCURRENT_LIMIT = 3; // 并发数（与服务器 MAX_CONCURRENT 一致）
...
async function runTest(sceneId) {
  const testLabel = `场景${sceneId}`;
  console.log(`\n${'─'.repeat(50)}`);
  console.log(`[${timestamp()}] 开始测试: 球星[3,4,5] × ${testLabel}`);
  ...
  try {
    ...
    if (pollResult.success) {
      ...
      const outputFile = path.resolve(outputDir, `球星345_场景${sceneId}_儿童.png`);
...
async function main() {
  ...
  console.log('  批量测试：球星[3,4,5] × 场景[1,2,3,4] × 儿童照片');
  console.log('='.repeat(60));
  console.log(`时间: ${new Date().toLocaleString('zh-CN')}`);
  console.log(`球星: Joshua Kimmich, Harry Kane, Luis Díaz`);
