 ...
 * 球星 2/4/8 (Olise, Kane, Neuer) × 场景 1/2/3/4 × 男性成人
 *
 * 包含球星参考图（人脸锚定修复验证）
 * 用法: node test_native_248_male.js
 */
...
// ─── 测试配置 ───
const STAR_IDS = ['2', '4', '8']; // Michael Olise, Harry Kane, Manuel Neuer
...
async function runTest(sceneId, userImage, userDescription) {
  ...
  try {
    ...
    if (PROMPT_ONLY) {
      ...
      // 球星参考图（人脸锚定）
      const playerRefs = getPlayerReferenceImages(STAR_IDS);
    ...
    // 4. 下载图片
    const outputDir = path.resolve(PROJECT_ROOT, '生成测试', '结果', '球星248男');
    ...
    const outputFile = path.resolve(outputDir, `球星${STAR_IDS.join('')}_场景${sceneId}_男_${modelTag}${modeTag}.png`);
...
async function main() {
  ...
  console.log(`  Native 模式批量测试：球星[${STAR_IDS.join(',')}] × 全场景 × 男性成人`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('  测试结果汇总 (Native / Seedream 4.5 + 球星参考图修复)');
  console.log('='.repeat(60));
